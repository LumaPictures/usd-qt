...
class HierarchyStandardContextMenuStrategy(object):
    ...
    def Construct(self, point):
        ...
        activateAction.triggered.connect(lambda checked=None: self.ActivateSelection())
        deactivateAction = menu.addAction("Deactivate %s" % name)
        deactivateAction.triggered.connect(lambda checked=None: self.DeactivateSelection())
        clearActiveAction = menu.addAction("Clear Activation for %s" % name)
        clearActiveAction.triggered.connect(lambda checked=None: self.ClearActiveForSelection())
        loadAction = menu.addAction("Load %s" % name)
        loadAction.triggered.connect(lambda checked=None: self.LoadSelection())
        unloadAction = menu.addAction("Unload %s" % name)
        unloadAction.triggered.connect(lambda checked=None: self.UnloadSelection())
